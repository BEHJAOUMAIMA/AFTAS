...
public class RankingRest {
    ...
    public ResponseEntity<?> registerMemberForCompetition(@Valid @RequestBody AddMemberRequestDTO addMemberRequestDTO) {
        ...
        return ResponseMessage.ok("Member registered successfully", new RankingResponseDTO(ranking.getMember().getId(), ranking.getCompetition().getId(), ranking.getScore(), ranking.getRank()));
    }
